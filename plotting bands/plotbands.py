...
parser = argparse.ArgumentParser(description='Plots the calculated band structure from syml file generated by questaal code. It uses the sym labels in syml.ctrl file. \nNote: Does not work with spin pol calculations yet !')
...
if os.path.isfile("syml."+inputs.ctrl) and os.path.isfile("bnds."+inputs.ctrl):
  go_plot(fl="bnds."+inputs.ctrl,sylmfile="syml."+inputs.ctrl,ymin=inputs.erange[0],
      ...
      units=inputs.units,outfile=inputs.out[0],verbose=inputs.v,yfontsize=inputs.yfontsize,xfontsize=inputs.xfontsize
     )
else:
